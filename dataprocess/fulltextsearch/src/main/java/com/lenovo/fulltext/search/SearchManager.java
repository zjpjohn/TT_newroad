package com.lenovo.fulltext.search;
...
import com.lenovo.fulltext.client.SearchServerClient;
...
public class SearchManager {
	...
	public List<?> simpleSearch(String query, SearchCriteria searchCriteria) {
		SearchRequestBuilder searchbuilder = searchClient.getESClient()
				.prepareSearch(searchCriteria.getIndex());
		...
		searchbuilder.addHighlightedField(query).setFrom(searchFrom)
				.setSize(searchRange).setExplain(true);
	...
	public List<?> conditionSearch(String query, SearchCriteria searchCriteria) {
		...
		if (fields != null) {
			...
			for (String field : fields) {
			  //queryBuilder.should(QueryBuilders.fuzzyQuery(field, query).fuzziness(fuzz).prefixLength(0));
			  queryBuilder.should(QueryBuilders.fuzzyQuery(field, query).minSimilarity(similarity));
			}
	...
	public List<?> multiSearch(String query, SearchCriteria... searchCriterias) {
		MultiSearchRequestBuilder multiSBuilder = searchClient.getESClient()
				.prepareMultiSearch();
