...
import com.newroad.user.sns.service.login.LenovoSTLogin;
...
public class AuthService {
  ...
  public ServiceResult<JSONObject> login(String apiVersion, Map<String, Object> param) {
    ...
    if (authType != 2) {
      user = snsDao.selectOne("user.getUserByAccountID", context.getThirdPartyAccount());
      if (user != null) {
    ...
    if (loginUser == null) {
      return new ServiceResult<JSONObject>(ReturnCode.AUTH_BAD, ControllerUtils.bulid("Fail to auth user info!"), null);
    }
  ...
  public ServiceResult<LoginUser> register(String apiVersion, Map<String, Object> param) throws Exception {
    ...
    if (StringUtils.isBlank(mobile) || StringUtils.isBlank(nickName)) {
    ...
    User user = new User(User.UserType.mobile.getCode(), nickName, password, nickName, mobile);
    context.setUser(user);
    saveUser(context);
    LoginUser loginUser = saveSession(context);
    if (loginUser == null) {
      return new ServiceResult<LoginUser>(ReturnCode.AUTH_BAD, ControllerUtils.bulid("Fail to auth user info!"), null);
    }
  ...
  public ServiceResult<String> checkUserExist(String apiVersion, Map<String, Object> param) throws Exception {
    ...
    String mobile = (String) param.get(MobileAuthLogin.MOBILE);
    String nickName = (String) param.get(MobileAuthLogin.NICKNAME);
    if (checkUserExist(mobile, nickName) == 0) {
  ...
  private Integer checkUserExist(String mobile, String nickName) {
    Map<String, Object> param = new HashMap<String, Object>(2);
    param.put(MobileAuthLogin.MOBILE, mobile);
    param.put(MobileAuthLogin.NICKNAME, nickName);
    logger.info("Check user exist:" + mobile + "," + nickName);
    User user = snsDao.selectOne("user.checkUserExist", param);
    if (user != null) {
      if (mobile.equals(user.getPhone())) {
        return 1;
      } else if (nickName.equals(user.getNickName()) || nickName.equals(user.getLoginName())) {
        return 2;
      }
    }
    return 0;
  ...
  public ServiceResult<JSONObject> check(Map<String, Object> param) throws Exception {
    String token = HttpMutualGetFilter.getRequestHeader(TokenUtil.AUTH_TOKEN);
    if (StringUtils.isBlank(token)) {
      return new ServiceResult<JSONObject>(ReturnCode.BAD_REQUEST, ControllerUtils.bulid("Parameter [" + TokenUtil.AUTH_TOKEN + "] not found!"),
          null);
    }
  ...
  private User checkUserUpdateStatus(LoginContext loginContext, User currentUser) {
    ...
    if (gender != null && !nickName.equals(currentUser.getNickName())) {
  ...
  private Account saveAccount(LoginContext context) {
    ...
    account.setSubSource(context.getThirdName());
...
//    login.setRealm(context.getRealm());
    ...
    login.setShortID(user.getShortID());
  ...
  public void setLenovost(LenovoSTLogin lenovost) {
